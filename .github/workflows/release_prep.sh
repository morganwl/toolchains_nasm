#!/usr/bin/env bash
# Build the rules_nasm release archive and emit release notes.
...
#
# Args:
#   $1: tag name (e.g. 0.5.0). Must match VERSION in version.bzl.
#
# Side effects:
#   Writes rules_nasm-${TAG}.tar.gz to the current directory. This
...
# ruleset). Note the `.bcr/presubmit.yml` test module is the repository
# root, so nothing else may be excluded here. The on-disk filename
# matches the URL in `.bcr/source.template.json` — the SLSA attestation
# subject must equal the published asset name.
#
...
# release_notes.template lives one directory up from this script.
NOTES="$(mktemp)"
sed "s#{version}#${TAG}#g" \
